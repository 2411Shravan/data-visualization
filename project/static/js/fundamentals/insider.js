const signup = document.querySelector('#insiderForm');
const tbody=document.querySelector('#tbody')
const hiddent=document.getElementById('hiddent')
signup.addEventListener('submit', (e) =>{
e.preventDefault();
const sym=signup['insiderName'].value;
console.log(sym.toUpperCase());

getInsider(sym);
        }
    )

async function getInsider(sym){
    const response = await fetch('https://finnhub.io/api/v1/stock/insider-transactions?symbol='+sym.toUpperCase()+'&token=c2vgio2ad3i9mrpv9i2g');
    const responseData= await response.json();
    console.log(responseData);
    showInsiders(responseData);
}

function showInsiders(response){
    const{data}=response;
    console.log(data);
    if(data.length<1){
        hiddent.innerHTML+="<h4 class='text-danger text-center'>Wrong Input</h4>"
    }
    else{
        data.forEach(dat=>{
            var change="";
            var share="";
            if (dat.change<0){
                change='danger';
                console.log(change);
            }
            else if (dat.change=0){
                change='primary';
                console.log(change);
            }
            else{
                change='success';
                console.log(change);
            }
    
            if (dat.share<0){
                share='danger';
                console.log(share);
            }
            else if (dat.share=0){
                share='primary';
                console.log(share);
            }
            else{
                share='success';
                console.log(share);
            }
    
            tbody.innerHTML += `
            <tr>
                <td><p class=text-${change}>${dat.change}</p></td>
                <td class=text-${share}>${dat.share}</td>
                <td>${dat.name}</td>
                <td>${dat.transactionPrice}</td>
            </tr>
            `
        })
    }
   
}